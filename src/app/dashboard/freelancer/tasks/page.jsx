...
function FreelancerTasksContent() {
  ...
  useEffect(() => {
    ...
    router.replace(`/dashboard/freelancer/tasks?${params.toString()}`, {
      scroll: false,
    });
  }, [category, search]);
  ...
  return (
    <div className="py-6 px-6">
      <h1 className="text-3xl font-bold text-primary mb-6">Browse Tasks</h1>

      {/* Filters */}
      <div className="flex flex-col gap-4 mb-6 bg-surface-raised p-5 rounded-2xl border border-border">
        <input
          type="text"
          placeholder="Search tasks by title..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full  rounded-xl border border-border px-4 py-3 bg-surface text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-accent transition-all"
        />
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <button
              ...
              className={`px-5 py-2 rounded-xl font-medium transition-all duration-200 border-2 ${
                category === cat
